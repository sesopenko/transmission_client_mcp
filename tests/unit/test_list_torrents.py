...
def _make_torrent(**overrides) -> MagicMock:
    ...
    t.peers_sending_to_us = 4
    t.peers_getting_from_us = 2
...
class TestListTorrentsNormalCase:
    ...
    def test_seeds_format(self):
        client = MagicMock()
        client.get_torrents.return_value = [_make_torrent(peers_sending_to_us=4, tracker_stats=[])]
        result = tools.list_torrents(client, _make_logger())
        assert result["torrents"][0]["seeds"] == "4/0"

    def test_peers_format(self):
        client = MagicMock()
        client.get_torrents.return_value = [_make_torrent(peers_getting_from_us=2, tracker_stats=[])]
        result = tools.list_torrents(client, _make_logger())
        assert result["torrents"][0]["peers"] == "2/0"
